...
try {
  ...
  setupModal('openSkye', 'skyeModal', 'closeSkye');
...
(function () {
  ...
  const listEl   = document.getElementById('skyeList');
  const searchEl = document.getElementById('skyeSearch');
  const countEl  = document.getElementById('skyeCount');
  if (!listEl || !searchEl) return;
...
} catch (e) { console.error('skye command list setup failed:', e); }
